...
Процедура ИсполняемыеСценарии() Экспорт
   ...
   ЮТТесты.ДобавитьТестовыйНабор(ИмяНабора(), "ИльинАВ, Остатки товаров, Взаиморасчеты, Приход, Расход").ВТранзакции()
   	.ДобавитьТест("ТестСозданияДокументов", "Проверяет, что документы записываются", "Позитив")
	.ДобавитьТест("ТестПроведенияДокумента", "Проверяет, что документы проводятся (есть движения)", "Позитив")
	.ДобавитьТест("ТестОстатковПоРегистрамНакопления", "Проверяет, что остатки по регистрам накопления выходят в ноль", "Позитив")
	;
...
Процедура ТестСозданияДокументов() Экспорт
	...
	ЮТест.ОжидаетЧто(Контекст.ПоступлениеДенег)
		.ИмеетТип("ДокументОбъект.ПоступлениеДенег")
		.Свойство("Ссылка").Заполнено();
...
Процедура ТестПроведенияДокумента() Экспорт
	...
	// 2. Выполнение функции.
	ПровестиДокументыКонтекста();
	
	// 3. Проверка результата.
	ЮТест.ОжидаетЧто(Контекст.ПриходТовара)
		.ИмеетТип("ДокументОбъект.ПриходТовара")
		.Свойство("Ссылка").Заполнено("Документ ПриходТовара успешно записан")
		.Свойство("Проведен").Равно(Истина, "Документ имеет признак Проведен")
...
Процедура ПодготовитьКонтекстНабора() Экспорт
...
Процедура ПровестиДокументыКонтекста()
	
	Контекст = ЮТест.КонтекстТестовогоНабора();
	
	Контекст.ПриходТовара.Записать(РежимЗаписиДокумента.Проведение);
	Контекст.РасходТовара.Записать(РежимЗаписиДокумента.Проведение);
	Контекст.Оплата.Записать(РежимЗаписиДокумента.Проведение);
	Контекст.ПоступлениеДенег.Записать(РежимЗаписиДокумента.Проведение);
